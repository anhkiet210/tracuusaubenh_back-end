 ...
 module.exports = (function () {
  ...
  function DecisionTreeID3() {
    ...
    if (numArgs === 1) {
      ...
      assert.ok(
        !!target && target.constructor === String,
        "`target` argument is expected to be a String. Check documentation on usage"
      );
      assert.ok(
        !!features && features.constructor === Array,
        "`features` argument is expected to be an Array<String>. Check documentation on usage"
      );
      ...
      throw new Error(
        "Invalid arguments passed to constructor. Check documentation on usage"
      );
    }
  }
  ...
  DecisionTreeID3.prototype = {
    ...
    train: function (data) {
      assert.ok(
        ...
        "`data` argument is expected to be an Array<Object>. Check documentation on usage"
      );
    ...
    predict: function (sample) {
      ...
      while (root.type !== NODE_TYPES.RESULT) {
        ...
        //   let childNode = _.filter(root.vals, (x) => {
        //       return x.name == sampleVal
        //   })

        if (childNode) {
          root.vals = childNode.child;
          arr = childNode
        } else {
  ...
  function createTree(data, target, features) {
    ...
    node.vals = _.map(possibleValues, function (featureVal) {
      const featureValDataSample = data.filter(
        (dataRow) => dataRow[bestFeatureName] == featureVal
      );
